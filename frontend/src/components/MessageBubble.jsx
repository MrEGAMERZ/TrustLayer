...
export default function MessageBubble({ message, onFollowup }) {
  ...
  if (message.role === "system") {
    if (message.isUpload && message.qualityData) {
      const q = message.qualityData;
      const scoreColor = q.quality_score >= 80 ? "#4ade80" : q.quality_score >= 50 ? "#fbbf24" : "#f87171";
      const scoreLabel = q.quality_score >= 80 ? "Good" : q.quality_score >= 50 ? "Fair" : "Poor";
      return (
        <div className="flex justify-center my-4 animate-fade-in">
          <div className="glass border border-emerald-500/25 rounded-2xl p-4 w-full max-w-sm">
            <div className="flex items-center gap-3 mb-3">
              <span className="text-lg">📥</span>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-white font-medium truncate">{message.filename}</p>
                <p className="text-[10px] text-gray-500 font-mono uppercase tracking-wider">Document ingested</p>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-[22px] font-light tabular-nums" style={{ color: scoreColor }}>{q.quality_score}</span>
                <span className="text-[9px] font-mono uppercase" style={{ color: scoreColor }}>{scoreLabel}</span>
              </div>
            </div>
            <div className="h-1 bg-white/[0.06] rounded-full overflow-hidden mb-3">
              <div className="h-full rounded-full transition-all duration-1000" style={{ width: q.quality_score + "%", background: scoreColor }} />
            </div>
            <div className="flex gap-3 text-[10px] font-mono text-gray-500">
              <span>⬡ {q.chunks_created} chunks</span>
              <span>📄 {q.total_pages} pages</span>
              {q.empty_pages > 0 && <span className="text-amber-400">⚠ {q.empty_pages} empty</span>}
            </div>
            {q.warnings?.length > 0 && (
              <p className="text-[10px] text-amber-400 mt-2 font-light">{q.warnings[0]}</p>
            )}
          </div>
        </div>
      );
    ...
    return (
      <div className="flex justify-center my-3 animate-fade-in">
        <div className="inline-flex items-start gap-3 glass border border-cyan-500/20 text-cyan-300 rounded-2xl px-5 py-3.5 max-w-sm text-[12px] font-mono shadow-lg">
          <span className="flex-shrink-0 mt-0.5">✓</span>
          <p className="font-light whitespace-pre-wrap leading-relaxed">{message.answer}</p>
        </div>
      </div>
    );
  }
            ...
            <div className="px-5 pb-4">
              <div className="bg-black/25 rounded-xl p-3.5 border border-white/[0.04] space-y-2">
                <div className="flex justify-between items-center text-[10px] font-mono uppercase tracking-wider">
                  ...
                  <span style={{ color: barColor }}>{pct}% source match</span>
                </div>
