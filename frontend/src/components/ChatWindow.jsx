...
export default function ChatWindow({ onNewSession }) {
  const [messages, setMessages]   = useState(() => {
    // Initializer: load from localStorage
    const saved = localStorage.getItem("trustlayer_chat");
  ...
  useEffect(() => {
    localStorage.setItem("trustlayer_chat", JSON.stringify(messages));
  ...
  const handleUploadSuccess = (filename, metadata) => {
    setMessages(prev => [
      ...
      {
        role: "system",
        answer: filename + " ingested",
        filename: filename,
        isUpload: true,
        qualityData: metadata || null,
  ...
  const handleSend = async (overrideText = null) => {
    ...
    // Auto-send logic: Clear input regardless of source (chip/manual)
    setInput("");
    setCharCount(0);
        ...
        while ((eolIndex = buffer.indexOf("\n\n")) >= 0) {
          ...
          if (line.startsWith("data: ")) {
            ...
            if (dataStr.startsWith("[DONE]")) {
              const metaStr = dataStr.substring(6);
              let meta = {};
              try { meta = JSON.parse(metaStr); } catch (e) {}
              
              if (meta.is_hallucinated) {
              ...
              setMessages(prev => {
                const newM = [...prev];
                newM[newM.length - 1].answer = answerText;
                return newM;
              });
            }
          }
